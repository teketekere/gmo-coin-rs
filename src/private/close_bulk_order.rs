...
/// 一括決済注文APIのパス。
const CLOSE_ORDER_API_PATH: &str = "/v1/closeBulkOrder";

/// 一括決済注文APIから返ってくるレスポンスを格納する構造体。
#[derive(Deserialize)]
pub struct CloseBulkOrder {
    /// ステータスコード。
    pub status: i16,
...
impl RestResponse<CloseBulkOrder> {
    /// 注文IDを取得する。
...
/// 一括決済注文APIを呼び出す。
pub async fn request_close_bulk_order(
...
mod tests {
    ...
    async fn test_market_order() {
        ...
        let resp = request_close_bulk_order(
            &http_client,
            &ExecutionType::Market,
            &Symbol::BtcJpy,
            &Side::Buy,
