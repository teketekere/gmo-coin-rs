...
/// また`GMO_COIN_LIMIT_PRICE`で指値価格を指定します。
///
...
async fn main() -> Result<(), Error> {
    let price: i64 = std::env::var("GMO_COIN_LIMIT_PRICE")?.parse().unwrap();
    let size = 0.01; // !!! 最小サイズ !!!

    let http_client = Reqwest;
    let private_api = PrivateAPI::<Reqwest> { http_client };
    let response = private_api
        .limit_order(&Symbol::BtcJpy, &Side::Buy, size, price)
        .await?;

    // 執行数量条件, ロスカットレートを指定する場合。
    // use gmo_coin_rs::time_in_force::TimeInForce;
    // let losscut_rate = ...;
    // let response = private_api.limit_order_with_options(&Symbol::BtcJpy, &Side::Buy, size, price, &TimeInForce::Fas, losscut_rate).await?;
